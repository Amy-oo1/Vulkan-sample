...
private:
	...
	void CleanUp(void) {
		...
		//vkDestroySemaphore(this->m_Logical_Device.get(), this->m_Render_Finished_Semaphore.get(), nullptr);
		this->m_Render_Finished_Semaphore.reset();

		//vkDestroySemaphore(this->m_Logical_Device.get(), this->m_Image_Available_Semaphore.get(), nullptr);
		this->m_Image_Available_Semaphore.reset();

		//vkDestroyFence(this->m_Logical_Device.get(), this->m_InFlight_Fence.get(), nullptr);
		this->m_InFlight_Fence.reset();
...
private:
	...
	void Record_Command_Buffer(VkCommandBuffer Command_Buffer, uint32_t Image_Index) {
		...
		if (VK_SUCCESS != vkBeginCommandBuffer(this->m_Command_Buffer, &Command_Buffer_Begin_Info))
...
private:
	...
	unique_ptr<VkSemaphore_T, function<void(VkSemaphore)>> m_Image_Available_Semaphore{ nullptr };
	unique_ptr<VkSemaphore_T, function<void(VkSemaphore)>> m_Render_Finished_Semaphore{ nullptr };

	unique_ptr<VkFence_T, function<void(VkFence)>> m_InFlight_Fence{ nullptr };
